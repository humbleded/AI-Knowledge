...
- **`/` vs `//`**：`/` 是真除法得 `float`（`404/100 = 4.04`），`//` 是整除得 `int`（`404//100 = 4`）。按状态码首位分类要用 `//`，否则非整百状态码（如 404）会因 `4.04 == 4` 为 `False` 而漏判。见 [[../Python/python-type-conversion]]。
...
- [[../Python/python-http-requests|Python HTTP 请求：URL、headers、status code 与 JSON（requests 库用法 · P0-09）]]
- [[debugging-triage|调试与错误恢复：系统化 triage]]（「按错误类型分流」里的网络错/超时）
- [[../Python/python-exceptions-debugging-testing|Python 异常、调试与单元测试]]
- [[../LLM/sampling-params-and-cost|采样参数与成本]]（调 API 的成本/超时意识）
- [[../Python/python-type-conversion|Python 类型转换]]（`/` vs `//`）
