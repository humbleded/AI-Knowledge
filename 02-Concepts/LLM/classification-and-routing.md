...
| **基于嵌入** Embedding-based | 输入向量与各路由向量的相似度 | （用 embedding 模型） | 语义路由，按含义而非关键词（见 [[embedding]]） |
| **基于 ML 模型** | 专门微调的判别式分类器 | ❌（推理时不调生成模型） | 路由逻辑编码在微调权重里 |

> PR2-03 只动手前两种：**规则版（基于规则）** 与 **模型版（基于 LLM）**——正好对应 [[summarizing-and-transforming|PR2-02]] 的「机械版 vs 模型版」。后两种属阶段 6 RAG / 后续。
...
1. **prompt 死命令「只输出一个标签词，前后不带别的字」**——否则模型回「这条属于投诉类。」，`== "投诉"` 失败（同 [[summarizing-and-transforming|锁 JSON]] 心法）。
2. **`.strip()`**：模型常返回 `"投诉\n"`，不洗则白名单/比较全失效（见 [[python-strip-and-cleaning-runtime-data]]）。
...
- ❌ 标签定义时手抖带尾随空格 `"账单 "` → ✅ 下游 `== "账单"` 因 `"账单 " != "账单"` 失败；**标签首尾永不留空格**（见 [[python-strip-and-cleaning-runtime-data]]）。
...
- [[summarizing-and-transforming|摘要与改写]]：同 PR2 系列，「机械版 vs 模型版」「锁格式只输出目标、别夹带人话」一脉相承。
- [[prompt-engineering-basics|Prompt 基础]]：分类 prompt 同样靠角色 + 清晰指令 + few-shot；few-shot 给边界例子才有价值。
- [[python-strip-and-cleaning-runtime-data|Python strip 与洗运行时脏数据]]：模型版 `.strip()` 洗 `"投诉\n"` 的依据。
- [[python-list-dict-set|Python list、dict、set]]：`RULES` 字典、`HANDLERS` 函数分发。
- [[message-roles-and-instruction-hierarchy|消息角色与指令优先级]]：分类指令放 `system`、待分类文本放 `user`。
