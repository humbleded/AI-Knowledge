...
> ⚠️ 口语里有时会把路由粗分成「LLM 理解型」和「非 LLM 确定型」两大类，但严格按实现方式看，常见是上表四类：LLM、嵌入、规则、专门训练的 ML 分类器。
...
```text
用户输入 -> 提取标签/意图 -> 进入对应路由 -> 该路由触发 JIT 检索 -> 回答/执行
```

它的价值在于**先缩小工作区，再装上下文**：

- 先分类：把“这是什么任务”判清楚，例如 `debug_issue`、`sales_query`、`doc_qa`。
- 再路由：不同标签走不同处理链、工具集、系统指令和安全策略。
- 再 JIT：由该路由决定要查什么资料，避免把代码、销售表、产品文档混在一起乱检索。

这能节省上下文，也能减少“检索污染”：问销售指标时不把代码库塞进来；修 bug 时不把营销文档塞进来。

工程上通常还要补两层：

- **公共基础上下文**：所有路由都需要的用户偏好、项目约定、权限边界，可以统一预加载。
- **反馈环**：路由处理中发现标签判错、参数缺失或证据不足时，可以重新分类或再次 JIT 检索。

规则路由在这个链条里很适合做内部确定分发：一旦上游已经抽出 `{intent: "refund"}` 这类结构化字段，后面用 `if-else` 或 `dict` 分发就很快、很稳。入口处面对自然语言长尾表达时，则更常用 LLM 路由或嵌入路由。
