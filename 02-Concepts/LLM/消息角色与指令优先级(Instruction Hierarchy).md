...
- [[调用 chat.completions]]：`messages` 怎么传、回复怎么取（`response.choices[0].message.content`）。
- [[分词(Tokenization)]]：`messages` 里所有角色的文本都会被 tokenize；system+user+assistant 全计入 token。
- [[自回归生成(Autoregressive)]]：模型续写下一句 assistant，就是逐 token 预测。
