...
- [[chat-completions-call|调用 LLM：chat.completions 与取回复]]：怎么发 `messages`、怎么取回复。
- [[message-roles-and-instruction-hierarchy|消息角色与指令优先级]]：system/user/assistant 的角色分工。
- [[streaming-output|流式输出：stream=True 与逐 chunk 处理]]：把这套多轮接成流式版（L1-04）；流式函数要 return 完整 answer 才能进 history。
- [[autoregressive-generation|Autoregressive Generation]]：`stream=True` 逐 token 返回的底层原因。
- [[python-file-json-serialization|Python 文件读写与 JSON 序列化]]：把 history 存盘续聊（dict 不丢类型，tuple 会变 list）。
