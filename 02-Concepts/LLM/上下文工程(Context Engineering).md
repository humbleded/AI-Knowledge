...
- 是**性能梯度**、非悬崖（越长越**缓慢**下滑，不是某长度突然崩）。机制：Transformer 每 token 两两关联，n² 增长 → 太长注意力被**摊薄**（见 [[注意力机制(Attention)]]）；训练语料短序列多、长上下文经验少。
...
- 工具走 **MVTS（最小可行工具集）**、few-shot 精选典型（见 [[提示工程基础(Prompt Engineering)]]）；总纲：**信息充分但紧致**。
...
## 召回 / 精确度：Compaction 的调参顺序

Compaction 的摘要不是越短越好，而是先回答两个问题：

- **召回 Recall**：该保留的关键信息保住了多少？例如架构决策、未解决 bug、复现步骤、当前约束。
- **精确度 Precision**：保留下来的内容里，有多少是真的有用？重复工具输出、旧日志、已解决的中间过程就是噪声。

安全顺序是：**先保召回，再提精确度**。先宁可多留一点，别把旧但关键的信息误删；确认关键点都在后，再合并重复输出、删掉过时工具结果和无关讨论。所谓“轻触式压缩”，就是优先清理深历史里的工具调用结果，而不是动核心决策。

## JIT、结构化笔记与渐进式披露

JIT 上下文和结构化笔记不是同一种东西：

- **JIT** 是动作时机：现在需要什么，就临时去文件、数据库、URL 或历史记录里拉什么。
- **结构化笔记** 是信息载体：把长期重要的信息沉淀成可维护的文档、TODO、决策记录或项目说明。

它们经常配合：新会话先读结构化笔记建立大局，再用 JIT 拉取当前任务需要的原始证据。任务完成后，再把新的稳定结论写回结构化笔记。

JIT 也可以看作**渐进式披露（progressive disclosure）**的一种实现：先只给轻量引用，例如文件路径、查询条件、URL；智能体运行过程中逐步读取必要内容，而不是一次性把所有资料塞进窗口。

引用的元数据本身也有价值：目录层级、文件名、大小、更新时间、路径里的 `tests/` 或 `src/`，都在暗示用途、时效和相关性。比如 `tests/test_utils.py` 更像测试辅助代码，`src/core/test_utils.py` 更像核心实现辅助代码。

预先放入 README、项目约定、AGENTS/CLAUDE 指南这类文件，属于“高价值基础上下文”。这些文件本身可以是结构化笔记；把它们预加载进窗口，则是上下文工程的选择。
...
- vs Compaction：trim＝**硬删**（旧的直接扔）；Compaction＝**软压**（旧的压成摘要保梗概）。二者**平级**；trim **不在**长时程三件套（硬删丢信息，长时程不够用）。详见 [[多轮对话与无状态记忆(Stateless Memory)]]。
...
- ⚠️ 工程坑 `str.split()` 按**空格**切**不按字**，中文整块算 1 词；`count_tokens` 中英混合会重复计中文、不精确（生产用真 tokenizer，详见 [[分词(Tokenization)]]）。
- ⚠️ 真调坑 `messages=text`（字符串）→ **400**；`messages` 要 `list of {"role","content"}`（同 [[调用 chat.completions]]）；把待总结文本包成 `[{system:总结指令},{user:text}]`。
- ⚠️ 实验设计坑：要演示「裁剪丢早期信息」，关键信息须**只在会被裁的早期出现**——若最近 N 条里复现（如末条复述名字），`name_in_trim` 会假性为 True，演示失效。

## 关联

- [[提示工程基础(Prompt Engineering)]]：上下文工程的**前身**；最小≠最短、few-shot 精选、长≠好同源。
- [[多轮对话与无状态记忆(Stateless Memory)]]：`trim_history` 是这里的朴素裁剪；记忆=每轮重发整盘 messages。
- [[摘要与改写(Summarize & Transform)]]：Compaction 用的就是「摘要＝有损压缩」；锁短靠 prompt 软约束 + 代码兜底。
- [[采样参数与成本(Sampling)]]：上下文越长 `prompt_tokens` 越多越贵、多轮每轮重发。
- [[注意力机制(Attention)]]：context rot 的架构根因（n² 注意力被摊薄）。
- [[分词(Tokenization)]]：`count_tokens` 毛估 vs 真 tokenizer。
- [[调用 chat.completions]]：真调摘要时 `messages` 结构与 `messages=text` 的 400 坑。
