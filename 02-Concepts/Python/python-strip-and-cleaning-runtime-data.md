...
- [[classification-and-routing|分类与路由]]：模型版三件套之一，洗 `call_model` 返回。
- [[python-and-or-short-circuit|Python and/or 短路]]：`delta.content or ""`、`x or 默认值` 是另一种「洗运行时脏值（None/空）」的手法。
- [[streaming-output|流式输出]]：`delta.content or ""` 兜 None 与 strip 洗脏同属「对模型返回做防御」。
- [[python-basic-data-types|Python 基础数据类型]]：str 不可变、方法返回新对象。
