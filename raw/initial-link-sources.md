---
type: source-index
topic: initial link sources
status: active
created: 2026-05-29
updated: 2026-05-29
source_type: extracted-links
tags:
  - Source
  - 知识库
  - ingest
---

# Initial Link Sources

这是从现有知识库 Markdown 链接中抽取出来的初始资料清单。它们可以作为 raw/source 层的第一批输入，用于后续 ingest、扩写课程页、补概念卡和补项目记录。

## Karpathy / LLM

- [karpathy/micrograd](https://github.com/karpathy/micrograd) -> [[repos/karpathy-micrograd]]
- [karpathy/nn-zero-to-hero](https://github.com/karpathy/nn-zero-to-hero) -> [[repos/karpathy-nn-zero-to-hero]]
- [Karpathy micrograd video](https://www.youtube.com/watch?v=VMj-3S1tku0) -> [[transcripts/karpathy-micrograd-video]]

## Python

- [廖雪峰 Python 教程主页](https://liaoxuefeng.com/books/python/)
- [廖雪峰 Python match 模式匹配](https://liaoxuefeng.com/books/python/basic/match/index.html)
- [CS50P Weeks](https://cs50.harvard.edu/python/weeks/)

## Agent / Skills

- [datawhalechina/Agent-Learning-Hub](https://github.com/datawhalechina/Agent-Learning-Hub)
- [datawhalechina/hello-agents](https://github.com/datawhalechina/hello-agents)
- [xindoo/agentic-design-patterns](https://github.com/xindoo/agentic-design-patterns)
- [huggingface/agents-course](https://github.com/huggingface/agents-course)
- [Hugging Face Agents Course](https://hf.co/learn/agents-course)
- [DeepLearning.AI - Agent Skills with Anthropic](https://www.deeplearning.ai/short-courses/agent-skills-with-anthropic/)
- [Claude Skills overview](https://claude.com/docs/skills/overview)
- [Agent Skills Dev 中文手册](https://agentskillsdev.com/)

## Obsidian / Knowledge Base References

- [kepano/obsidian-skills](https://github.com/kepano/obsidian-skills)
- [Ar9av/obsidian-wiki](https://github.com/Ar9av/obsidian-wiki)
- [Obsidian Hub](https://github.com/obsidian-community/obsidian-hub)
- [SoRobby/ObsidianStarterVault](https://github.com/SoRobby/ObsidianStarterVault)

## Existing Compile Targets

- [[../03-Courses/LLM/Karpathy-Zero-to-Hero/README]]
- [[../03-Courses/Python/Python-Basics/廖雪峰 Python 教程]]
- [[../03-Courses/Python/Python-Basics/CS50P Weeks 中文学习索引]]
- [[../03-Courses/Agent/AI-Agent-Learning/README]]
- [[../01-Maps/GitHub 参考资源]]
- [[../01-Maps/AI 知识库总地图]]

## Next Ingest Rules

- 每次真正阅读或整理一个链接后，新增独立 raw source note 或更新已有 source note。
- 不把链接清单当成最终知识；它只是输入队列。
- 每次从链接生成知识页后，更新 [[../log]]。
